...
function parseCommand(text) {
  ...
  m = text.match(/^(\d+)번\s+(.+)$/);
  if (m) return { command: 'register_employee', employee_no: m[1], name: m[2] };
  ...
  if (m) return { command: 'remove_days', employee_no: m[1], amount: Number(m[2]) };
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!message) return res.json(kakaoSimpleText('message 필드 없음'));
